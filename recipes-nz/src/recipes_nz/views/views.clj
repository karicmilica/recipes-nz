(ns recipes-nz.views.views
  (:require [clojure.data.json :as json]
           [net.cgrand.enlive-html :as html]))

(html/deftemplate login-template "recipes_nz/views/login.html" [title]
   [:title] (html/content title))

(html/defsnippet menu-snippet "recipes_nz/views/menu.html" [[:#menu]] [ingrCategories]
...
(html/defsnippet search-snippet "recipes_nz/views/searchView.html" [[:#results]]  [title recipes]
...
(html/defsnippet recommedation-snippet "recipes_nz/views/recommendationlist.html" [[:#recommendation]] 
  [recommendations]
...
(html/deftemplate search-recipes-template "recipes_nz/views/layout.html"
...
(html/defsnippet recipe-snippet "recipes_nz/views/recipeView.html"
   [[:#recipe]] 
  [recipe]
     ...
     [:#current-rating] (if (not (nil? (:userRecipeRating recipe))) 
                          (html/set-attr :style (str "width: " (* (:userRecipeRating recipe) 25) "px;"))) 
     [:ul.ingredients :li] (html/clone-for [item (:ingredients recipe)] 
                                           (html/content item)))

(html/deftemplate recipe-template "recipes_nz/views/layout.html"
  [recipe categories]
    [:title] (html/content (:title recipe))
    [:#left] (html/content (menu-snippet categories))
    [:#content] (html/content (recipe-snippet recipe)))

(html/deftemplate registration-template "recipes_nz/views/register.html"
 ([title msg]
   [:title] (html/content title)
