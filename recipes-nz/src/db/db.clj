...
(defn userRecipeRating [userId recipeId]
  (let [k (keyword recipeId)
        user (mc/find-one-as-map "users" {(str "recipeRatings." recipeId) { $gt 0} :_id userId} [(str "recipeRatings." recipeId)])
...
(defn averageRecipeRating [recipeId]
  (let [k (keyword recipeId)
        users (mc/find-maps "users" {(str "recipeRatings." recipeId) { $gt 0}} ["username" (str "recipeRatings." recipeId)])
        c (count users)
...
(defn createUser [username password name email]
...
(defn addRecipeRatingForUser [userId recipeId rate]
  (mc/update-by-id "users" userId {$set {(str "recipeRatings." recipeId) rate}}))
